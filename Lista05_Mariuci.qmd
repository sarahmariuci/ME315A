...
```
Nota: dividir por 3.0 garante que o resultado seja retornado em formato decimal/ponto flutuante
...
```{sql connection=con}
SELECT PNAME, FEE AS TAXA_ATUAL, (FEE + 50.0) / 2.0 AS TAXA_AJUSTADA FROM PRESERVE
```

4. Apresente os valores médio, máximo e mínimo de ACRES para todas as reservas naturais localizadas no Arizona.
```{sql connection=con}
SELECT AVG(ACRES) AS VALOR_MEDIO, MAX(ACRES) AS VALOR_MAXIMO, MIN(ACRES) AS VALOR_MINIMO FROM PRESERVE
WHERE STATE = 'AZ'
```
...
```{sql connection=con}
SELECT COUNT(PNAME) AS TOTAL_REGISTROS, COUNT(DISTINCT PNAME) AS TOTAL_DISTINTOS FROM PRESERVE
```
...
```{sql connection=con}
SELECT SUM(SALARY) AS SOMA_SALARIO, AVG(SALARY) AS MEDIA_SALARIO, MAX(SALARY) AS SALARIO_MAXIMO, MIN(SALARY) AS SALARIO_MINIMO FROM EMPLOYEE
```

12. Quantos funcionários trabalham no Departamento 20?
```{sql connection=con}
SELECT COUNT(*) AS QTD_FUNCIONARIOS FROM EMPLOYEE
WHERE DNO = 20
```
